...
class Tiket extends CI_Controller
{
	...
	public function cekjadwal($tgl = '', $asl = '', $tjn = '')
	{
		...
		$data['bus'] = $this->input->get('nama_bus');
	...
	public function beforebeli($jadwal = "", $asal = '', $tanggal = '')
	{
		...
		if ($this->session->userdata('username')) {
			$id = $jadwal;
			$asal = $asal;
			$data['nama_bus'] = $this->db->query("SELECT * FROM tbl_jadwal LEFT JOIN tbl_bus on tbl_jadwal.kd_bus = tbl_bus.nama_bus WHERE kd_jadwal ='" . $id . "'")->row_array();
			...
			$data['kursi'] = $this->db->query("SELECT no_kursi_order FROM tbl_order WHERE kd_jadwal = '" . $data['jadwal']['kd_jadwal'] . "' AND asal_order = '" . $asal . "'")->result_array();
	...
	public function gettiket($value = '')
	{
		...
		for ($i = 0; $i < $count; $i++) {
			$simpan = array(
				'kd_order' => $getkode,
				'kd_tiket' => 'T' . $getkode . $kd_jadwal . str_replace('-', '', $tglberangkat),
				'kd_jadwal'	=> $kd_jadwal,
	...
	public function payment($id = '')
	{
		...
		$data['tiket'] = $sqlcek;
		$data['bus'] = $this->db->query("SELECT * FROM tbl_bus WHERE nama_bus = '" . $id . "'")->result_array();
		// die(print_r($data));
	...
	public function checkout($value = '')
	{
		...
		$data['tiket'] = $value;
		$data['count'] = $this->db->query("SELECT count(kd_order) FROM tbl_order WHERE kd_order ='" . $value . "'")->row_array();
		$data['bayar'] = $this->db->query("SELECT * FROM tbl_order LEFT JOIN tbl_jadwal on tbl_order.kd_jadwal = tbl_jadwal.kd_jadwal LEFT JOIN tbl_tujuan on tbl_jadwal.kd_tujuan = tbl_tujuan.kd_tujuan LEFT JOIN tbl_bank on tbl_order.kd_bank = tbl_bank.kd_bank WHERE kd_order ='" . $value . "'")->row_array();
		$data['nomrek'] = $this->db->query("SELECT * FROM tbl_order LEFT JOIN tbl_jadwal on tbl_order.kd_jadwal = tbl_jadwal.kd_jadwal LEFT JOIN tbl_tujuan on tbl_jadwal.kd_tujuan = tbl_tujuan.kd_tujuan LEFT JOIN tbl_bank on tbl_order.kd_bank = tbl_bank.kd_bank WHERE kd_order ='" . $value . "'")->row_array();


		$this->load->view('frontend/chekout', $data);
	}
	...
	{
		$data['cetak'] = $this->db->query("SELECT * FROM tbl_bus ")->result_array();


		$order = $id;
		$data['cetak'] = $this->db->query("SELECT * FROM tbl_order LEFT JOIN tbl_jadwal on tbl_order.kd_bus = tbl_jadwal.kd_bus LEFT JOIN tbl_bus on tbl_order.kd_bus = tbl_bus.nama_bus LEFT JOIN tbl_jadwal on tbl_order.kd_jadwal = tbl_jadwal.kd_jadwal LEFT JOIN tbl_tujuan on tbl_jadwal.kd_tujuan = tbl_tujuan.kd_tujuan WHERE kd_order ='" . $id . "'")->result_array();

		$tiket = $this->db->query("SELECT email_pelanggan FROM tbl_pelanggan WHERE kd_pelanggan ='" . $data['cetak'][0]['kd_pelanggan'] . "'")->row_array();
		$this->load->view('frontend/cetaktiket', $data);
		/*die(print_r($tiket));*/
	}
	public function listbus()
	{
		...
		$this->load->view('frontend/bus', $data);
	}
}
